...
Map::Map() : m_pTile(NULL), m_pEditor(NULL), m_width(0), m_isDragging(false), m_isScaling(false)
{
...
void Map::endEdit(bool apply)
{
    ...
    if (apply)
    {
        ...
        if (m_pEditor->m_isSetter)
            ...
            {
                CCNode * tile = m_pTile->getChildByTag(m_pEditor->m_setVec[i]);
