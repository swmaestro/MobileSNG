...
public class BuildingRProductComplete extends HttpServlet{
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try{
				...
				{	
					...
					if(Temp.getRequestState()==BuildingRequestInfo.REQUEST_PRODUCTING)
					{					
						Vresults = (List<VillageBuilding>) q2.execute(owner, index);
						if(Vresults.get(0).getVBState() == VillageBuilding.BUILDING_ENDPRODUCT)
						{
							Temp.setRequestState(BuildingRequestInfo.REQUEST_PRODUCT_COMPLETE);
							Vresults.get(0).setVBState(VillageBuilding.BUILDING_REQUEST_WAITING);
							inputChk = true;
						}
					}
				}
