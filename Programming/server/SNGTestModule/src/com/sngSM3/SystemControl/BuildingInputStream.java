...
//ver , index, name, cost, profit, BTime, PTime, size, level 

public class BuildingInputStream extends HttpServlet{
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		int ptime=(int)new Integer(req.getParameter("ptime"));  
		int size=(int)new Integer(req.getParameter("size"));  
		int level=(int)new Integer(req.getParameter("level"));  
		float cexp=(Float)new Float(req.getParameter("CExp")); 
		float pexp=(Float)new Float(req.getParameter("PExp")); 
		PersistenceManager pm = PMF.getPMF().getPersistenceManager();
		Boolean inputChk=false;
		
		try{
				BuildingInfo building = new BuildingInfo( ver,index, name, costA,costB,profitA,profitB,btime,ptime,size,level,cexp,pexp);
